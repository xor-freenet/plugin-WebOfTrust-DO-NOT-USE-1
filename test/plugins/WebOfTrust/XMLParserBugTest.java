...
/**
 * This is a copy of XML_FAIL2.java of https://bugs.frenetproject.org/view.php?id=4850
 * It has been adapted to run as a JUnit test so we can use Travis CI to prove that the underlying
 * JRE bug has been fixed in all recent Java version.
 * Original documentation of XML_FAIL2 follows: 
 * 
 * This class demonstrates a bug in the XML parser of Java.
 * 
 * It contains a valid XML document according to the W3 XML validator (http://validator.w3.org)
 * When we parse the XML and try to obtain the value of all "Identity"-attributes of the "Trust"-Elements,
 * we will notice that one value is not the expected USK@.../ URI but rather part of the raw XML.
 * The output of such a failure to obtain the correct Identity-value will be:
 * Parsing failure: Instead of USK URI I got: " Value="100"/><Trust Comment="" Identity="USK@bK9LQT2mqilUY4DxwmlKvKi5Jfwf4Brr~EQGa8RLIRU,Ni90~c4q-VNFrxFS7cDimQwC
 * 
 * This does not seem to be an error in the XML.
 * 
 * I first tried to produce a minimal sample XML which triggers this bug. But I was unable to: 
 * Logically, one would try to remove all of the Trust-Elements except the one which triggers the bug.
 * So I started to remove them line by line and test whether the bug does not disappear.
 * At first look  there are quite a few which cause it to not disappear and also some which cause it to disappear.
 * Unfortunately, at the second look you notice there seems to be some sort of weird cross-dependency between them.
 * So I decided to just give you the full original XML.
 * Notice: This XML was obtained through a peer to peer network, it is part of the protocol.
 * It IS possible that someone hand-generated this as a deliberate attack.
 * You should check for things like evil, bogus UTF characters etc.
 * 
 * For me this is reproduceable with:
 * java version "1.6.0_24"
 * Java(TM) SE Runtime Environment (build 1.6.0_24-b07)
 * Java HotSpot(TM) 64-Bit Server VM (build 19.1-b02, mixed mode)
 * 
 * This currently blocks the release of the WebOfTrust subproject of Freenet in a rather critical
 * way because I am unable to identify which part of the XML actually triggers the bug, so I
 * cannot do a workaround. This is rather annoying because the project has been under development
 * for over three years and this is very last bug which is preventing it from release :( 
 */
public class XMLParserBugTest {
	...
	public void testIdentityXMLParsing() throws IOException, SAXException,
		...
		for(int i = 0; i < trustList.getLength(); ++i) {
			...
			assertTrue("Parsing failure: Instead of USK URI I got: " + identity,
				keyPattern.matcher(identity).matches());
		}
	}
}
